async function setWeatherBackground() {
  const lat = 39.0639; // Grand Junction, CO approx
  const lon = -108.5506;
  ...
  if (code === 0) cls = 'weather-clear'; // clear sky
  else if ([1, 2, 3].includes(code)) cls = 'weather-clear'; // mainly clear to partly cloudy
